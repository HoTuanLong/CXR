...
class complete_model(pl.LightningModule):
    ...
    def validation_epoch_end(self, outs):
        ...
        # print("average_val_loss: " + str(loss.item()))
        # wandb.log({"phase2_valid_loss": loss.item()}, step=self.epoch_idx) nhớ bật lên
    ...
    def test_various_metrics(self, testset, save_path=None):
        ...
        if accuracies["precision_at_1"] > self.best_score:
            ...
            torch.save(self.model.state_dict(), '/home/ubuntu/long.ht/cxr-patient-reidentification/ckps/chexpert_checkpoint_client.pth')
...
def train_FL():
    ...
    return trainer50, model50, data_module

trainer50, model50, data_module = train_FL()

class FlowerClient(fl.client.NumPyClient):
    
    def get_parameters(self, config):
        return [val.cpu().numpy() for _, val in model50.state_dict().items()]

    def set_parameters(self, parameters):
        params_dict = zip(model50.state_dict().keys(), parameters)
        state_dict = OrderedDict({k: torch.tensor(v) for k, v in params_dict})
        model50.load_state_dict(state_dict, strict=True)
...
def main() -> None:
    ...
    wandb.login()
    wandb.init(project="CXR-Patient-Client", entity="longht", name="CheXpert")
