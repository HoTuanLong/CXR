...
def main():
    ...
    parser = argparse.ArgumentParser('Patient Retrieval Main Test')
    ...
    size_list = [1024, 950, 800, 512, 224]
    # initialize Network
    ...
    for size in size_list:
        print("size: ", size)
        with open(config["main_data_save_path"], 'a+') as f:
            f.write(' "Resolution: %s\n' % size)
        # initialize Network
        test_set = eval_Dataset(image_root, csv_test, csv_test_singles, [size, size])

        #  load the new state dict
        model50.test_various_metrics(test_set, config["main_data_save_path"])
