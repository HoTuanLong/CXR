...
class complete_model(pl.LightningModule):
    ...
    def test_epoch_end(self, outs):
        loss = torch.stack(outs).mean()
        # metrics = {"average_test_loss: ", loss.item()}
        print("average_test_loss: ", loss.item())
        self.test_various_metrics(self.test_set_val)
    ...
    def test_various_metrics(self, testset, save_path=None):
        ...
        metrics = {"test_MAP@R_test": accuracies["mean_average_precision_at_r"],
                   "test_r_precision": accuracies["r_precision"],
                   "test_precision_at_1": accuracies["precision_at_1"],
                   "mean__test_distance": mean_distance.item(),
                   "max_test_distance": max_distance.item()
                   }
...
trainer50, model50, data_module, model_save_path = train_FL()
