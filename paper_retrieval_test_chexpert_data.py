...
class complete_model_test(pl.LightningModule):
    ...
    # the actual testing algorithm, done every epoch, but can also be called outside of training with any given testset as
    # a parameter
...
def main():
    ...
    parser = argparse.ArgumentParser('Patient Retrieval CheXpert Test')
    ...
    csv_chexpert = config["csv_chexpert_data"]
    image_root_chexpert = config["image_root_chexpert"]
    ...
    # initialize Network
    test_set = eval_Dataset(image_root_chexpert, csv_chexpert, size=[1024, 1024])

    #  load the new state dict
    model50.test_various_metrics(test_set, config["chexpert_data_save_path"])
